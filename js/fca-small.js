  ...
  AttributesForm = RC({
    render: function() {
      return form({
        ...
        label({}, [R.DOM.b({}, 'Attributes'), ' (a|b): ']), input({
          ref: 'input',
          ...
          autoFocus: true,
          type: 'search'
        }), br({}), small({}, 'Premise: all implications between sets of attributes are true unless there is a counterexample.')
      ]);
    },
    onChange: function(e) {
  ...
  RulesList = RC({
    render: function() {
      ...
      describeRule = function(_arg) {
        ...
        if (from.length) {
          return "" + (attrText(from)) + " → " + (attrText(to));
        } else {
          return "∅ → " + (attrText(to));
        }
      };
      ...
      return div(hideIf(!this.props.attributes.length, {
        className: 'implications'
      }), [
        form({}, [
          label({}, [R.DOM.b({}, 'Implications')]), option('as table', this.state.tableView, (function(_this) {
            return function(e) {
              return _this.setState({
                tableView: e.target.checked
              });
            };
          })(this)), option('accepted last', this.state.confirmedBelow, (function(_this) {
            return function(e) {
              return _this.setState({
                confirmedBelow: e.target.checked
              });
            };
          })(this)), option('negate', this.props.autoNegate.get(), this.props.autoNegate.set)
        ]), items.length ? this.state.tableView ? table({
          className: 'rules'
        }, [
          thead({}, tr({}, [
          ...
          className: 'rules'
        }, items) : p({}, R.DOM.i({}, ['No more nontrivial implications.']))
      ]);
    },
